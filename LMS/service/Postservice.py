...
class PostService:
    ...
    def save_post(member_id, title, content, files=None, upload_folder='uploads/'):
        ...
        try:
                ...
                if files:
                    for file in files:
                        if file and file.filename != '':
                            ...
                            ext = origin_name.split('.', 1)[1].lower()
                            save_name = f"{uuid.uuid4().hex}.{ext}" # hex -> 16진수 / 상단에 import uuid
            ...
            conn.close() # save_post(member_id, title, content, files, upload_folder) -> True / False로 리턴한다.
    ...
    def get_post_detail(post_id):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                conn.commit()
                return post, files # post는 post에 있는 자료 / files는 첨부파일에 대한 자료
        finally:
            conn.close()
    ...
    def delete_post(post_id, upload_folder='uploads/'):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                # 삭제시 cascade 옵션을 하지 않으면 자식 테이블 데이터를 선 삭제 후 부모 테이블 데이터를 삭제한다.
